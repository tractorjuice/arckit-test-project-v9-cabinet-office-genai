...
**Required Files**:
1. `projects/{project-name}/requirements.md` - ML-related requirements (FR/NFR)
2. `projects/{project-name}/data-model.md` - Training data, features

**Optional Files** (read if available):
3. `.arckit/memory/architecture-principles.md` - AI/ML principles
4. `projects/{project-name}/ai-playbook-assessment.md` - Responsible AI context
5. `projects/{project-name}/atrs-record.md` - Algorithmic transparency
6. `projects/{project-name}/jsp-936-assessment.md` - MOD AI assurance
7. `projects/{project-name}/research-findings.md` - ML platform research
...
1. **Save file** to `projects/{project-name}/mlops-strategy.md`
...
**File**: projects/{project-name}/mlops-strategy.md
...
"⚠️ No ML-related requirements found. Please ensure requirements.md includes ML use cases (search for 'model', 'ML', 'AI', 'predict')."

### If No Data Model:
"⚠️ Data model not found. Training data understanding is important for MLOps. Consider running `/arckit.data-model` first."
...
- `ARC-[PROJECT_ID]-MLOPS-v[VERSION]` → Document ID
